...
@Disabled
@Autonomous(name = "testRedClose", group = "main")
public class testRedClose extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        if(tip_autonomie==0)
        {
            roadRunner.init(-10, 36, 0, true, 0, 1);

            while (roadRunner.isRunning)
                roadRunner.run();

            roadRunner.init(20, 27, 0, true, Geometry.pi/6, 1);

            while (roadRunner.isRunning)
                roadRunner.run();

            roadRunner.init(-16, 55, 0, true, Geometry.pi/4, 1);
        ...
        {
            roadRunner.init(20, 20, 0, true, 0, 1);

            while (roadRunner.isRunning)
                roadRunner.run();
            roadRunner.init(10, 64, 0, true, 0, 1);
        ...
        {
            roadRunner.init(24, 50, 0, true, 0, 1);
        ...
        if(tip_autonomie==0)
        {
            roadRunner.init(116, 72, 0, true, Geometry.pi / 2, 1);
        }
        else if(tip_autonomie==1)
        {
            roadRunner.init(116, 64, 0, true, Geometry.pi / 2, 1);
        }
        else if(tip_autonomie==2)
        {
            roadRunner.init(116, 52, 0, true, Geometry.pi / 2, 1);
        }
        ...
        roadRunner.init(110, 10, 0, true, Geometry.pi / 2, 1);
